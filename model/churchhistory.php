...
class ChurchHistoryModel extends BaseModel {
	
	public function __construct(){
		parent::__construct();
		$this->table_name = "churchhistory";
	}
	
	public function get(){
		$query = "SELECT * FROM churchhistory";		
		$record = $this->db->query($query);		
		return $record;		
	}
	
	public function getChurchHistoryDetailsById($id=0){
		$query = "SELECT * FROM churchhistory WHERE id=".$id;		
		$record = $this->db->query($query);		
		return $record;		
	}
	...
	public function delete($id=0){
		$query = "DELETE FROM churchhistory WHERE id = " . $id;
		$this->db->query($query);		
		return True;		
	}
}
